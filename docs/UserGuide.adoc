...
*** Example: `find title\ software engineering` will list all modules with titles containing `"software" *or* "engineering"`.
** `desc\` : Find all modules with *description* that contains the given *keywords(s)*.
*** Example: `find desc\ software engineering` will list all modules with description containing `"software" *or* "engineering"`.
...
*or* `cs3` with description containing `"software"` *or* `"engineering"` offered in semester 1.
...
****
[NOTE]
Keywords are searched using *or* relation while predicates are seached using
*and* relations.
For example, `find mod\ cs ma title\ linear` will produce results of all modules with (module code containing `cs` *or* `ma`) *and* (title containing `linear`)
To get the best search results, use as many filters and as little keywords as possible.
